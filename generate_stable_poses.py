    ...
    # for obj in Path("./CAD_model/UT1113").rglob('*.obj'):
        ...
        for i in tqdm(range(iter)):
            ...
            for part in obj_queue:
                ...
                R_obj_in_world = np.array(part_pose[0:3, 0:3])
                R_cam_in_world = np.eye(3)

                R_obj_in_cam = np.matmul(R_obj_in_world, np.linalg.inv(R_cam_in_world))
                
                from scipy.spatial.transform import Rotation
                R_object = Rotation.from_matrix(R_obj_in_cam)
                quaternion = R_object.as_quat()
                q_obj_in_cam = np.insert(quaternion, 0, [0, 0, 0])

                Rot_mat.append(R_obj_in_cam)
                Quaternions.append(q_obj_in_cam)
        ...
        print('#'*80)
        print(f"The Quaternion pose list of {catogory} is \n", Quaternions)
        np.savez_compressed(args.output_dir + '/' + catogory + '_R', Rotations = Rot_mat)
        np.savez_compressed(args.output_dir + '/' + catogory + '_Q', Quaternions = Quaternions)